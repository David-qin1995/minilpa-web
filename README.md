...
本系统支持多人同时在线，实时显示在线用户数量：

- 👥 **按 IP 统计**：根据客户端真实 IP 地址统计在线人数（同一 IP 只算 1 人）
- 🟢 **连接状态**：绿色指示灯表示 WebSocket 连接正常
- 🔒 **独立操作**：每个用户编辑各自的配置文件，互不影响
- ⚡ **实时更新**：在线人数实时更新，无延迟

### 技术实现

- 使用 **Socket.io** 实现 WebSocket 双向通信
- 后端根据 IP 地址去重统计（支持代理/负载均衡）
- 前端自动重连机制，确保连接稳定
- 支持 20+ 人同时在线操作

### IP 获取优先级

1. `x-forwarded-for` 头（通过 Nginx/代理时）
2. `x-real-ip` 头
3. 直接连接的 socket IP
